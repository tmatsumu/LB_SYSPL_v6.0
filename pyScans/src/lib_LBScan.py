...
#    phi_boresight = mylib.wraparound_2pi(2.*pi*freq_boresight*time_i)
    ...
    # convert from theta (physics) to lat here and apply wrap, modified in 2022-2-1 by TM.
    theta_out = pi/2. - theta_out

    # add isnan check, if there is, insert the value from the neigbor sample
    ind = np.isnan(theta_out)
    ind2 = np.where(( ind == True ))
    if len(ind2[0]) !=0: 
        if len(ind2[0]) == 1: 
            theta_out[ind2[0]] = theta_out[ind2[0]-1]
            phi_out[ind2[0]] = phi_out[ind2[0]-1]
        if len(ind2[0]) > 1: print 'check this out, there are many nan from this calculation!!'
    ############################################################################3
    if option_gen_ptg == True:
        fname=dir_out+'/ptg/LBPTG_latlon_eclip_'+str(int(today_julian))
        tmpstr = time_julian[0]-2400000.5
...
#; ----------------------------------------------------------------------------
def gen_scan_c_mod2_stephwp( theta_antisun, theta_boresight, freq_antisun, freq_boresight, \
                                 total_time, today_julian, sample_rate, dir_out, filename, \
                                 title, nside, runtime_i, option_gen_ptg, hwp_step_rad):
