...
if option_sh!='y':
    ...
    dir_out = gl.path_out
    step_hwp_rad = g.step_hwp_rad/radeg
...
if option_sh=='y':
    ...
    dir_out = gl.path_out
    step_hwp_rad = np.float(sys.argv[11])/radeg
...
print '   step HWP angle:', step_hwp_rad*radeg
print '' 
...
f.write('%s %s \n' % ('dir_out, ', dir_out))
f.write('%s %f \n' % ('step HWP angle [degs], ', step_hwp_rad*radeg))
f.close() 
...
for i in range(0,ydays):
    ...
    mapout = liblb.gen_scan_c_mod2_stephwp(theta_antisun, theta_boresight, freq_antisun, freq_boresight, \
                                               total_time, today_julian, sample_rate, dir_out, filename, \
                                               title, nside, runtime_i, option_gen_ptg, step_hwp_rad*np.float(i))
